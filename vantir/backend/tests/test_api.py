...
import split_ontology.api as api_module
...
def test_reverse_address_endpoint_returns_lookup_payload(monkeypatch):
    def fake_lookup_reverse_address(*, lat, lon):
        return {
            "type": "ReverseAddress",
            "status": "ok",
            "label": "Marmontova ul., Split",
            "source": "test",
            "lat": lat,
            "lon": lon,
        }

    monkeypatch.setattr(api_module, "lookup_reverse_address", fake_lookup_reverse_address)
    response = TestClient(api_module.create_app()).get(
        "/api/addresses/reverse?lat=43.51&lon=16.44"
    )

    assert response.status_code == 200
    assert response.json()["label"] == "Marmontova ul., Split"
